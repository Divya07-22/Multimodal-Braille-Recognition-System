...
export default function Login() {
  ...
  const inputClass = `w-full px-4 py-3 rounded-2xl outline-none transition-all text-sm ${settings.highContrast
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-16">
      ...
      <motion.div
        ...
        className={`relative w-full max-w-md rounded-3xl border p-8 ${settings.highContrast
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className={`block text-xs font-bold uppercase tracking-widest mb-2 ${settings.highContrast ? 'text-yellow-400' : 'text-white/40'
              }`}>Email</label>
            <input
              type="email"
              value={form.email}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button
                ...
                className={`absolute right-3 top-1/2 -translate-y-1/2 p-1.5 rounded-lg transition-all ${settings.highContrast
          ...
          <div className="flex justify-end">
            <Link
              to="/forgot-password"
              className={`text-xs font-medium transition-all ${settings.highContrast
          ...
          <button
            type="submit"
            disabled={isLoading}
            className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl font-bold transition-all disabled:opacity-60 disabled:cursor-not-allowed mt-2 ${settings.highContrast
        ...
        <div className={`mt-8 pt-6 border-t text-center ${settings.highContrast ? 'border-yellow-400' : 'border-white/10'}`}>
          <p className={`text-sm ${settings.highContrast ? 'text-yellow-200' : 'text-white/50'}`}>
            ...
            <Link
              to="/register"
              className={`font-bold transition-all ${settings.highContrast
                  ? 'text-yellow-400 hover:text-yellow-300'
                  : 'text-violet-400 hover:text-violet-300'
                }`}
            >
        ...
        <div className={`mt-4 p-3 rounded-2xl text-xs text-center ${settings.highContrast ? 'bg-yellow-400/10 text-yellow-300' : 'bg-white/5 text-white/30'
          }`}>
