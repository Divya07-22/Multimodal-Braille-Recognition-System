...
class AnalyticsService:
    async def get_platform_stats(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        total_inf = await db.execute(
            select(func.count()).select_from(InferenceResult)
        )
        avg_conf = await db.execute(
            select(func.avg(InferenceResult.confidence_score))
        )
        avg_time = await db.execute(
            select(func.avg(InferenceResult.processing_time_ms))
        )
    ...
    async def get_user_stats(
        ...
        total_inf = await db.execute(
            ...
            .where(InferenceResult.user_id == user_id)
        )
        avg_conf = await db.execute(
