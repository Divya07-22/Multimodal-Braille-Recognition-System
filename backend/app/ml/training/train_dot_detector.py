...
def train_dot_detector(
    ...
    checkpoint = ModelCheckpoint(
        artifacts_dir, "dot_detector", monitor="val_loss", mode="min"
    )
    tracker = MetricsTracker(
        os.path.join(artifacts_dir, "dot_detector_metrics.json")
    )
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        exact_match = (
            ((all_preds == all_labels).all(dim=1)).float().mean().item()
        )
        ...
        logger.info(
            f"Epoch {epoch}/{num_epochs} | Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Dot Acc: {dot_accuracy:.4f} | Exact: {exact_match:.4f}"
        )
